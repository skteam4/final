...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        Alarm # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        Alarm
    </v-card-title >
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field type="number" label="BookingId" v-model="value.bookingId"/>
          </div>
          <div class="grey--text ml-4" v-else>
            BookingId :  {{value.bookingId }}
          </div>
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field type="number" label="CcId" v-model="value.ccId"/>
          </div>
          <div class="grey--text ml-4" v-else>
            CcId :  {{value.ccId }}
          </div>
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="CcName" v-model="value.ccName"/>
          </div>
          <div class="grey--text ml-4" v-else>
            CcName :  {{value.ccName }}
          </div>
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="BookingStatus" v-model="value.bookingStatus"/>
          </div>
          <div class="grey--text ml-4" v-else>
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/alarms'), this.value)
          }else{
